...
type Props = {
  ...
  session: Session;
  /** Catalog products for explain cards. Pass the latest list known to the page. */
...
const GREETING =
  "Hi — let’s dig in. Here’s the shortlist, then I’ll walk you through the top picks.";

export function ChatPanel({ session, products, shortlist }: Props) {
  ...
  // Hide every user bubble — /results is a one-sided briefing from the
  // assistant. The user can still type follow-ups; the model receives
  // them, but we don't echo them back as bubbles.
  ...
  return (
    ...
    >
      <Conversation className="min-h-0 flex-1">
        <ConversationContent className="flex min-h-full flex-col justify-end space-y-0.5 px-4 py-4">
          ...
          {shortlist.length > 0 && (
            <div className="max-w-[64%] sm:max-w-[52%]">
      ...
      <div className="border-t border-[#442c2d]/10 px-3 py-2">
        <div className="flex items-end gap-2 rounded-lg px-1 focus-within:ring-2 focus-within:ring-primary/40">
          ...
          <button
            ...
            onClick={onSubmit}
            className="inline-flex size-8 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
